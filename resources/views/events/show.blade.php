                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                    @foreach ($relatedEvents as $relatedEvent)
                        <div class="card bg-base-100 shadow-xl hover:shadow-2xl transition-shadow duration-200">
                            <figure class="h-48">
                                <img src="{{ $relatedEvent->image_url }}" alt="{{ $relatedEvent->judul }}" class="w-full h-full object-cover">
                            </figure>
                            <div class="card-body">
                                <h3 class="card-title text-base">{{ $relatedEvent->judul }}</h3>
                                <div class="flex items-center gap-2 text-sm text-gray-600">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"></path>
                                    </svg>
                                    <span>{{ \Carbon\Carbon::parse($relatedEvent->tanggal_waktu)->format('d M Y') }}</span>
                                </div>
                                <div class="flex items-center gap-2 text-sm text-gray-600">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z"></path>
                                    ...
                                    <span>{{ $relatedEvent->lokasi }}</span>
                                </div>
                                <div class="card-actions justify-end mt-4">
                                    <a href="{{ route('events.show', $relatedEvent) }}" class="btn btn-primary btn-sm">Lihat Detail</a>
                                </div>
                            </div>
                        </div>
                    @endforeach
